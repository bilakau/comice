...
function redirectTo404() {
  contentArea.innerHTML = `<div class="text-center py-40 text-red-500">Error 404: Halaman tidak ditemukan atau API bermasalah.</div>`;
}
...
async function showHome(push = true) {
  ...
  if (!data || (!data.komikList && !data.komikPopuler)) { redirectTo404(); return; }

  const populer = data.komikPopuler || [];
  const terbaru = data.komikList || [];
      ...
      <div class="flex overflow-x-auto gap-4 hide-scroll pb-4 -mx-4 px-4 md:mx-0 md:px-0">
        ${populer.map(item => `
          <div class="min-w-[150px] md:min-w-[200px] cursor-pointer card-hover relative rounded-2xl overflow-hidden group"
            ...
            <span class="type-badge type-default bg-black/80"><i class="fa fa-star text-amber-400"></i> ${item.rating || 'Hot'}</span>
            ...
            <div class="absolute bottom-0 left-0 p-3 z-20 w-full">
              ...
              <p class="text-amber-400 text-xs font-semibold mt-1 line-clamp-1">${item.author || ''}</p>
            </div>
          </div>
        `).join('')}
      </div>
            ...
            <div class="bg-zinc-900/40 border border-white/5 rounded-xl overflow-hidden cursor-pointer hover:border-amber-500/50 transition group"
              ...
              <div class="p-3">
                ...
                <div class="flex justify-between items-center mt-3">
                  <span class="text-[10px] bg-white/5 px-2 py-1 rounded text-amber-500 font-bold">${item.chapters?.[0]?.title || 'Ch.?'}</span>
                  <span class="text-[10px] text-gray-500 line-clamp-1 ml-2 text-right">${item.chapters?.[0]?.date || ''}</span>
                </div>
              </div>
            </div>
          `).join('')}
...
async function showOngoing(page = 1) {
  ...
  setLoading();
  // Filter API baru agak berbeda, kita pakai library page
  const data = await fetchAPI(`${API_BASE}/library?page=${page}`);
  renderGrid(data, "Daftar Komik (Library)", "showOngoing");
}

async function showCompleted(page = 1) {
  updateURL('/completed'); resetNavs();
  setLoading();
  // Karena tidak ada status eksplisit di dokumentasi API baru, fallback ke library
  const data = await fetchAPI(`${API_BASE}/library?page=${page}`);
  renderGrid(data, "Daftar Komik", "showCompleted");
}
...
async function applyAdvancedFilter() {
  ...
  renderGrid(data, "Daftar Komik", null);
}
      ...
      ${list.map(item => `
        <div class="bg-zinc-900/40 rounded-xl overflow-hidden border border-white/5 card-hover cursor-pointer relative group"
          ...
          <div class="p-3 text-center">
            ...
            <p class="text-[10px] text-amber-500 mt-1 font-medium">${item.chapters?.[0]?.title || item.rating || 'Baca Sekarang'}</p>
          </div>
        </div>
      `).join('')}
    </div>
...
async function showDetail(idOrSlug, push = true) {
  ...
  // Clean Title
  const cleanTitle = res.title ? res.title.replace(/Komik\s+/ig, '').trim() : "Unknown";
  currentComicContext = { slug, title: cleanTitle, image: res.image };
  ...
  // Use new res.description for Synopsis
  const synopsisText = res.description || "Sinopsis tidak tersedia.";
  const isLongSynopsis = synopsisText.length > 250;
  
  // Use new nested details
  const detailType = res.detail?.type || res.type || 'Comic';
  const detailStatus = res.detail?.status || res.status || 'Berjalan';
    ...
    <div class="relative z-10 flex flex-col md:flex-row gap-8 lg:gap-12 mt-4 animate-fade-in">
      ...
      <div class="md:w-[280px] flex-shrink-0 mx-auto md:mx-0 w-full max-w-[280px]">
        <div class="relative group">
          <span class="type-badge ${getTypeClass(detailType)} scale-110 top-4 left-4 shadow-lg">${detailType}</span>
        ...
        <div class="flex flex-col gap-3 mt-6">
          ...
          <button onclick="toggleBookmark('${slug}', '${String(cleanTitle).replace(/'/g, "")}', '${res.image}')" id="btn-bookmark"
      ...
      <div class="flex-1 min-w-0">
        <h1 class="text-3xl md:text-5xl font-extrabold mb-4 leading-tight bg-clip-text text-transparent bg-gradient-to-r from-white to-gray-400">${cleanTitle}</h1>
        ...
        <div class="flex flex-wrap gap-3 mb-6">
          ...
          <div class="glass px-4 py-1.5 rounded-lg flex items-center gap-2 text-xs font-bold text-green-400 border border-green-500/20">
            <i class="fa fa-circle text-[6px]"></i> ${detailStatus}
          </div>
        ...
        <div class="flex flex-wrap gap-2 mb-6">
          ${res.genres ? res.genres.map(g => `
            <span class="text-gray-400 text-xs px-3 py-1 rounded-full border border-white/10 bg-white/5">
              ${g.name || g.title}
            </span>`).join('') : ''}
        </div>
  ...
  saveHistory(slug, cleanTitle, res.image);
  window.scrollTo(0, 0);
}
    ...
    return `
      <div onclick="safeReadChapter('${ch.slug}', '${comicSlug}')"
        ...
        <div class="flex items-center gap-3 shrink-0">
          <span class="text-[10px] text-gray-500 hidden sm:block">${ch.releaseTime || ''}</span>
          <div class="text-[10px] text-gray-500 bg-black/20 px-2 py-1 rounded group-hover:bg-amber-500 group-hover:text-black transition font-bold">
            Baca
          </div>
        </div>
      </div>
...
async function readChapter(chIdOrSlug, comicSlug = null, push = true) {
  ...
  try {
    ...
    // Use allChapterSlug from API new format
    let finalComicSlug = res.allChapterSlug || comicSlug;
    ...
    const cleanTitle = comicTitle.replace(/Komik\s+/ig, '').trim();
    const chapterLabel = normalizeChapterLabel(res.title || chSlug);
    const headerTitle = `${cleanTitle} - ${chapterLabel}`;
    ...
    } else if (res.komikInfo?.chapters) {
    ...
    // Convert object {id, url} to simple array of string URL
    const imgs = res.images ? res.images.map(img => img.url || img) : [];
    ...
    if (finalComicSlug) {
      saveHistory(finalComicSlug, cleanTitle, currentComicContext?.image, chSlug, chapterLabel);
    }
