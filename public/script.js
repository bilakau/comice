...
// ==================== FETCH DENGAN PROXY BARU ====================
async function fetchAPI(url) {
  try {
    const response = await fetch(API_PROXY + encodeURIComponent(url));
    const data = await response.json();
    // Proxy baru mengembalikan { success: true, result: { status, content } }
    if (data.success && data.result && data.result.content) {
      return data.result.content; // data asli dari API
    }
    return null;
  } catch (e) {
...
async function loadGenres() {
  // Asumsi endpoint /genres masih tersedia
...
// ==================== HALAMAN UTAMA (HOME) DENGAN ENDPOINT /latest ====================
async function showHome(push = true) {
  ...
  const komikPopuler = data.komikPopuler || [];
  ...
  contentArea.innerHTML = `
    <section class="mb-12">
      ...
      <div class="flex overflow-x-auto gap-4 hide-scroll pb-4 -mx-4 px-4 md:mx-0 md:px-0">
        ${komikPopuler.slice(0, 10).map(item => `
          <div class="min-w-[150px] md:min-w-[200px] cursor-pointer card-hover relative rounded-2xl overflow-hidden group"
            ...
            <span class="type-badge type-default">Populer</span>
          ...
          ${komikList.slice(0, 15).map(item => `
            <div class="bg-zinc-900/40 border border-white/5 rounded-xl overflow-hidden cursor-pointer hover:border-amber-500/50 transition group"
              ...
              <div class="relative h-48 overflow-hidden">
                <span class="type-badge ${getTypeClass(item.type)} bottom-2 left-2 top-auto">${item.type || 'Komik'}</span>
          ...
          ${komikPopuler.slice(0, 10).map(item => `
            <div class="flex gap-4 bg-zinc-900/30 p-2 rounded-xl cursor-pointer hover:bg-white/5 transition border border-transparent hover:border-white/10"
              ...
              <div class="flex-1 flex flex-col justify-center overflow-hidden">
                ...
                <p class="text-[8px] text-gray-500 truncate">${item.author || ''}</p>
              </div>
            </div>
          `).join('')}
        </div>
...
// ==================== ONGOING & COMPLETED (asumsi endpoint /list masih ada) ====================
...
// ==================== RENDER GRID (mendukung berbagai struktur data) ====================
function renderGrid(data, title, funcName, extraArg = null) {
  // data bisa berupa objek dengan properti komikList, data, atau array langsung
  let list = [];
  let pagination = null;
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="flex flex-wrap gap-3 mb-6">
          ...
          <div class="glass px-4 py-1.5 rounded-lg flex items-center gap-2 text-xs font-bold text-green-400 border border-green-500/20">
            <i class="fa fa-circle text-[6px]"></i> ${res.detail?.status || res.status}
          </div>
        ...
        <div class="flex flex-wrap gap-2 mb-6">
          ${res.genres ? res.genres.map(g => {
            // bersihkan slug genre (dari "/genres/action" menjadi "action")
            const genreSlug = g.slug.replace('/genres/', '');
            return `<span onclick="showGenre('${genreSlug}')" class="cursor-pointer hover:text-amber-500 transition text-gray-400 text-xs px-3 py-1 rounded-full border border-white/10 hover:border-amber-500/50 bg-white/5">
              ${g.name}
            </span>`;
          }).join('') : ''}
        </div>
